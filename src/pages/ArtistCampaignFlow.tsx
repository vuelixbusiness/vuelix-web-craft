...
import { Upload, Music, Target, DollarSign, Eye, Heart, Calendar as CalendarIcon, PlayCircle, Users, TrendingUp, ArrowLeft, ArrowRight, Rocket } from "lucide-react";
...
const ArtistCampaignFlow = () => {
  ...
  const genres = ["Hip Hop", "Pop", "R&B", "Rock", "Electronic", "Country", "Jazz", "Reggae", "Latin", "Indie", "Folk", "Classical"];
  const platforms = [{
    id: "tiktok",
    name: "TikTok",
    icon: "🎵"
  ...
  const canContinue = (step: number) => {
    switch (step) {
      case 1:
        return (campaignData.songFile || campaignData.songLink) && campaignData.campaignType && campaignData.genre && campaignData.platforms.length > 0;
      case 2:
        return campaignData.payoutType && campaignData.payoutRate && campaignData.instructions;
      case 3:
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="mb-8">
            ...
            <div className="flex space-x-2">
              {[1, 2, 3].map(step => <div key={step} className={`flex-1 h-2 rounded-full transition-smooth ${step <= currentStep ? 'bg-primary' : 'bg-secondary'}`} />)}
            </div>
          ...
          {currentStep === 1 && <Card className="border-2">
              <CardHeader>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-2 border-dashed border-border rounded-lg p-8 text-center">
                    ...
                    <Input placeholder="Paste SoundCloud or Spotify link" value={campaignData.songLink || ''} onChange={e => updateCampaignData('songLink', e.target.value)} />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={campaignData.genre} onValueChange={value => updateCampaignData('genre', value)}>
                    <SelectTrigger className="bg-background border-border">
                      <SelectValue placeholder="Select genre" />
                    </SelectTrigger>
                    <SelectContent className="bg-background border-border z-50 max-h-60">
                      {genres.map(genre => <SelectItem key={genre} value={genre.toLowerCase()}>
                          {genre}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {platforms.map(platform => <Card key={platform.id} className={`cursor-pointer transition-smooth border-2 ${campaignData.platforms.includes(platform.id) ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'}`} onClick={() => togglePlatform(platform.id)}>
                ...
                <Button className="w-full" disabled={!canContinue(1)} onClick={() => setCurrentStep(2)}>
                  Continue to Rewards
          ...
          {currentStep === 2 && <Card className="border-2">
              <CardHeader>
                <CardTitle className="flex items-center space-x-2">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={campaignData.payoutType} onValueChange={value => updateCampaignData('payoutType', value)}>
                    <SelectTrigger className="bg-background border-border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input type="number" placeholder="0.00" className="pl-10" value={campaignData.payoutRate || ''} onChange={e => updateCampaignData('payoutRate', parseFloat(e.target.value))} />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input type="number" placeholder="0.00" className="pl-10" value={campaignData.vipBonus || ''} onChange={e => updateCampaignData('vipBonus', parseFloat(e.target.value))} />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea placeholder="Provide clear instructions for creators. Include any specific requirements, hashtags to use, or creative direction..." className="min-h-[120px]" value={campaignData.instructions || ''} onChange={e => updateCampaignData('instructions', e.target.value)} />
                </div>

                {/* Reference Links */}
                <div className="space-y-2">
                  <Label className="text-base font-medium">Reference Links (Optional)</Label>
                  <Textarea placeholder="Add links to example videos, mood boards, or other reference materials..." className="min-h-[80px]" value={campaignData.referenceLinks || ''} onChange={e => updateCampaignData('referenceLinks', e.target.value)} />
                </div>

                {/* Approval Required */}
                <div className="flex items-center space-x-2">
                  <Switch id="approval" checked={campaignData.approvalRequired || false} onCheckedChange={checked => updateCampaignData('approvalRequired', checked)} />
                ...
                <div className="flex space-x-4">
                  ...
                  <Button className="flex-1" disabled={!canContinue(2)} onClick={() => setCurrentStep(3)}>
                    Continue to Preview
                    <ArrowRight className="w-4 h-4 ml-2" />
                  </Button>
                </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <div className="flex gap-1">
                            {campaignData.platforms.map(platform => <Badge key={platform} variant="outline" className="text-xs">
                                {platform}
                              </Badge>)}
                          </div>
                        </div>
              ...
              <Card className="border-2">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !campaignData.endDate && "text-muted-foreground")}>
                      ...
                      <PopoverContent className="w-auto p-0 bg-background border-border z-50" align="start">
                        <Calendar mode="single" selected={campaignData.endDate} onSelect={date => updateCampaignData('endDate', date)} disabled={date => date < new Date()} initialFocus className={cn("p-3 pointer-events-auto")} />
                      </PopoverContent>
                    </Popover>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <DollarSign className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                      <Input type="number" placeholder="0.00" className="pl-10" value={campaignData.budget || ''} onChange={e => updateCampaignData('budget', parseFloat(e.target.value))} />
                    </div>
                  </div>

                  {/* Budget Summary */}
                  {campaignData.budget && campaignData.payoutRate && <Card className="bg-primary/5 border-primary/20">
                  ...
                  <div className="flex space-x-4">
                    ...
                    <Button variant="hero" className="flex-1" disabled={!canContinue(3)}>
